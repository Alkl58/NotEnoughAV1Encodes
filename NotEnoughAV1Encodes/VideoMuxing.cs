            ...
            if (SmallFunctions.Cancel.CancelAll == false)
            {
                ...
                if (MainWindow.audioEncoding == false)
                {
                    ...
                    else
                    {
                        ...
                        ffmpegCommand = "/C ffmpeg.exe -y -f concat -safe 0 -i " + '\u0022' + Path.Combine(MainWindow.tempPath, "Chunks", "chunks.txt") + '\u0022' + " -c copy " + '\u0022' + Path.Combine(MainWindow.tempPath, "temp.mkv") + '\u0022';
                        SmallFunctions.Logging("VideoMuxing() Command: " + ffmpegCommand);
                        await Task.Run(() => SmallFunctions.ExecuteFfmpegTask(ffmpegCommand));
                        ...
                        ProcessStartInfo startInfo = new ProcessStartInfo
                        {
                            ...
                            Arguments = "/C mkvmerge.exe --output " + '\u0022' + MainWindow.videoOutput + '\u0022' + " --no-track-tags --no-global-tags --language 0:und --default-track 0:yes " + '\u0022' + Path.Combine(MainWindow.tempPath, "temp.mkv") + '\u0022' + " " + MainWindow.subtitleMuxingInput
                        };
